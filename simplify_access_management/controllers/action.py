...
class Action(Action):
    ...
    def run(self, action_id, context=None):
        ...
        actions_and_prints = []
        if res:
            cids = request.httprequest.cookies.get('cids') and [int(num) for num in request.httprequest.cookies.get('cids').split('-')]  or request.env.company.id
            for access in request.env['remove.action'].search([('access_management_id.company_ids','in',cids),('access_management_id','in',request.env.user.access_management_ids.ids),('model_id.model','=',res.get('res_model'))]):
    ...
    def load(self, action_id, additional_context=None):
        ...
        if res:
            cids = request.httprequest.cookies.get('cids') and [int(num) for num in request.httprequest.cookies.get('cids').split('-')]  or request.env.company.id
            for view_data in set(request.env['remove.action'].sudo().search([('view_data_ids','!=',False),('access_management_id.company_ids','in',cids),('access_management_id','in',request.env.user.access_management_ids.ids),('model_id.model','=',res.get('res_model'))]).mapped('view_data_ids.techname')):
    ...
    def web_client(self, s_action=None, **kw):
        ...
        if not kw.get('debug') or kw.get('debug') != "0":
            ...
            if access_management.id:
                new_url = request.httprequest.base_url + "?debug=0"
