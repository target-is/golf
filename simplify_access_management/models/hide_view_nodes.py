# -*- coding: utf-8 -*-
# Hide View Nodes - Custom Model for hiding view elements like buttons, pages, and links in Odoo 18
#
# This model enables the hiding of specific elements (buttons, tabs/pages, kanban links) in views based on the selected model and user access.
#
# Note: Please ensure to test your custom module with Odoo 18 as the view and button extraction logic may have minor updates due to changes in Odoo's frontend framework.
from odoo import fields, models, api, _
from odoo.tools.translate import TranslationModuleReader
from lxml import etree

class HideViewNodes(models.Model):
    # This class is used to manage the nodes (buttons, tabs, links) in a view, allowing for the hiding or modification of UI components in a model's views based on user-specific access control.
    # It interacts with the 'store.model.nodes' model to track and manage these components, ensuring that only authorized users can view specific elements.
...
class StoreModelNodes(models.Model):\
    # This model is used to store metadata for UI components (buttons, pages, links) within Odoo models.
    # It is used by the 'HideViewNodes' model to define and manage UI elements that can be hidden or customized based on user permissions.
